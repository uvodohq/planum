import { Alert, Flex, Button } from '@uvodohq/planum'

import { ArgsTable, Story, Canvas, Meta } from '@storybook/addon-docs'

<Meta title="Components/Alert" component={Alert} />

# Alert

- [Overview](#overview)
- [Variants](#variants)
- [Props](#props)

## Overview

Alert banners show pressing and high-signal messages, such as system alerts. They are meant to be noticed and prompt users to take action.Alerts are rendered at the top of the page, below the header.Alert banners are rendered as a div with a role=<b>"alert"</b> attribute. Alert shows a close button by default is false. Can be closed by clicking on the close button.

## Variants

### Alerts with message

These alerts consist of a message and a close button. You can pass a message to the alert component. The close button is true by default. By clicking on the close button, the alert will be closed.

<Canvas>
  <Story name="With message">
    <Flex
      css={{ flexDirection: 'column', gap: 32, py: 30, width: '100%' }}
      id="alert">
      <Alert
        message="Molestie et, consectetur condimentum facilisis elementum duis sit."
        variant="primary"
        closable
      />
      <Alert
        message="Molestie et, consectetur condimentum facilisis elementum duis sit."
        variant="success"
        closable
      />
      <Alert
        message="Molestie et, consectetur condimentum facilisis elementum duis sit."
        variant="warning"
        closable
      />
      <Alert
        message="Molestie et, consectetur condimentum facilisis elementum duis sit."
        variant="error"
        closable
      />
      <Alert
        message="Molestie et, consectetur condimentum facilisis elementum duis sit."
        variant="info"
        closable
      />
    </Flex>
  </Story>
</Canvas>

### Alerts with title

These alerts consist of a title, a message, and without a close button. You can pass a title to the alert component. The close button can be shown after closable prop is true .

<Canvas>
  <Story name="With title">
    <Flex
      css={{ flexDirection: 'column', gap: 32, py: 30, width: '100%' }}
      id="alert">
      <Alert
        title="Non porttitor nisl volutpat ac imperdiet."
        message="Molestie et, consectetur condimentum facilisis elementum duis sit. Lobortis proin lectus amet ullamcorper."
        variant="primary"
        closable={false}
      />
      <Alert
        title="Non porttitor nisl volutpat ac imperdiet."
        message="Molestie et, consectetur condimentum facilisis elementum duis sit. Lobortis proin lectus amet ullamcorper."
        variant="success"
        closable={false}
      />
      <Alert
        title="Non porttitor nisl volutpat ac imperdiet."
        message="Molestie et, consectetur condimentum facilisis elementum duis sit. Lobortis proin lectus amet ullamcorper."
        variant="warning"
        closable={false}
      />
      <Alert
        title="Non porttitor nisl volutpat ac imperdiet."
        message="Molestie et, consectetur condimentum facilisis elementum duis sit. Lobortis proin lectus amet ullamcorper."
        variant="error"
        closable={false}
      />
      <Alert
        title="Non porttitor nisl volutpat ac imperdiet."
        message="Molestie et, consectetur condimentum facilisis elementum duis sit. Lobortis proin lectus amet ullamcorper."
        variant="info"
        closable={false}
      />
    </Flex>
  </Story>
</Canvas>

### Alerts with actions

These alerts consist of a message, and an action buttons. You can pass a message and an action buttons to the alert component. Action buttons have variants and icons. Also, these variants are same as the alert variants.

<Canvas>
  <Story name="With Actions">
    <Flex css={{ flexDirection: 'column', gap: 32, py: 30, width: '100%' }}>
      <Alert
        title="Non porttitor nisl volutpat ac imperdiet."
        message="Molestie et, consectetur condimentum facilisis elementum duis sit. Lobortis proin lectus amet ullamcorper."
        variant="primary"
        closable
        actions={
          <Button size="min" variant={'primary'} compact>
            Learn more
          </Button>
        }
      />
      <Alert
        title="Non porttitor nisl volutpat ac imperdiet."
        message="Molestie et, consectetur condimentum facilisis elementum duis sit. Lobortis proin lectus amet ullamcorper."
        variant="success"
        closable
        actions={
          <Button size="min" variant={'success'} compact>
            Learn more
          </Button>
        }
      />
      <Alert
        title="Non porttitor nisl volutpat ac imperdiet."
        message="Molestie et, consectetur condimentum facilisis elementum duis sit. Lobortis proin lectus amet ullamcorper."
        variant="warning"
        closable
        actions={
          <Button size="min" variant={'warning'} compact>
            Learn more
          </Button>
        }
      />
      <Alert
        title="Non porttitor nisl volutpat ac imperdiet."
        message="Molestie et, consectetur condimentum facilisis elementum duis sit. Lobortis proin lectus amet ullamcorper."
        variant="error"
        closable
        actions={
          <Button size="min" variant={'error'} compact>
            Learn more
          </Button>
        }
      />
      <Alert
        title="Non porttitor nisl volutpat ac imperdiet."
        message="Molestie et, consectetur condimentum facilisis elementum duis sit. Lobortis proin lectus amet ullamcorper."
        variant="info"
        closable
        actions={
          <Button size="min" variant="info" compact>
            Learn more
          </Button>
        }
      />
    </Flex>
  </Story>
</Canvas>

## Props

<ArgsTable of={Alert} />
